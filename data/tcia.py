...
class DatasetTCIA(Dataset):
    def __init__(self, datapath, fold, transform, split, shot, num=600):
        ...
        self.base_path = os.path.join(datapath, 'TCIA_SegPC')
        self.img_path = os.path.join(self.base_path, 'Images')
        self.ann_path = os.path.join(self.base_path, 'labels')
    ...
    def load_frame(self, query_name, support_names):
        ...
        query_id = os.path.basename(query_name).replace('.png', '.png')
        query_img = Image.open(os.path.join(self.img_path, query_id)).convert('RGB')
        support_ids = [os.path.basename(name).replace('.png', '.png') for name in support_names]
