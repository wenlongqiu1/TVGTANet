r""" Kvasir few-shot semantic segmentation dataset """
import os
import glob
...
class DatasetKvasir(Dataset):
    def __init__(self, datapath, fold, transform, split, shot, num=600):
        self.split = split
        self.benchmark = 'kvasir'
        self.shot = shot
        self.num = num

        self.base_path = os.path.join(datapath, 'Kvasir')
        self.img_path = os.path.join(self.base_path, 'imagesTr')
        self.ann_path = os.path.join(self.base_path, 'labelsTr')
    ...
    def load_frame(self, query_name, support_names):
        ...
        query_id = os.path.basename(query_name).replace('.png', '.jpg')
        query_img = Image.open(os.path.join(self.img_path, query_id)).convert('RGB')
        support_ids = [os.path.basename(name).replace('.png', '.jpg') for name in support_names]
