...
class DatasetFSS(Dataset):
    ...
    def load_frame(self, query_name, support_names):
        ...
        query_id = query_name.split('\\')[-1].split('.')[0]
        query_mask_path = os.path.join(os.path.dirname(query_name), query_id) + '.png'
        query_pascal_path = os.path.join(os.path.dirname(query_name), query_id) + '_pascal.png'

        support_ids = [name.split('\\')[-1].split('.')[0] for name in support_names]
        support_masks = [os.path.join(os.path.dirname(support_name), sid) + '.png' for support_name, sid in zip(support_names, support_ids)]
        support_pascals = [os.path.join(os.path.dirname(support_name), sid) + '_pascal.png' for support_name, sid in zip(support_names, support_ids)]

        query_mask = self.read_mask(query_mask_path)
        query_pascal = self.read_mask(query_pascal_path)
        support_masks = [self.read_mask(mask) for mask in support_masks]
        support_pascals = [self.read_mask(pascal) for pascal in support_pascals]
