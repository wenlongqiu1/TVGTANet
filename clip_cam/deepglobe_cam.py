...
from clip_text import new_class_names_D6, BACKGROUND_CATEGORY_D6
import argparse
...
def perform(label_list, label_id_list, process_id, dataset_list, args, model, bg_text_features, fg_text_features, cam):
    # the obtained pseudo labels need to be placed separately in folders of different categories,Need to adjust different clip_text
    cam_out_dir = r"D:\CDFSSDataset\Target domains\Deepglobe\1\test\Pascal"
    ...
    for im_idx, im in enumerate(tqdm(databin)):
        ...
        for image in ms_imgs:
            ...
            for idx, label in enumerate(label_list):
                ...
                # Process attention matrix, normalize row and column values
                aff_mat = attn_weight
                ...
                # Further matrix multiplication enhances features
                for _ in range(1):
                ...
                cam_refined_highres = scale_cam_image([cam_refined], (400, 400))[0]
            ...
            for i, cam_image in enumerate(refined_cam_to_save):
                ...
                cv2.imwrite(save_path, (cam_image_np * 255).astype(np.uint8))  # 保存为PNG格式
...
if __name__ == "__main__":
    ...
    parser.add_argument('--img_root', type=str, default=r'D:\CDFSSDataset\Target domains\Deepglobe\1\test\origin')
    parser.add_argument('--model', type=str, default=r'..clip/ViT-B-16.pt')
    parser.add_argument('--num_workers', type=int, default=1)
    args = parser.parse_args()
    label_list = ['Barren lan']
    label_id_list = [0]  # 类别的ID
    ...
    bg_text_features = zeroshot_classifier(BACKGROUND_CATEGORY_D6, ['a clean origami {}.'], model)#['a rendering of a weird {}.'], model)
    fg_text_features = zeroshot_classifier(new_class_names_D6 , ['a clean origami {}.'], model)#['a rendering of a weird {}.'], model)
