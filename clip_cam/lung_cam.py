...
from clip_text import class_names_lung , new_class_names_lung , BACKGROUND_CATEGORY_lung
import argparse
...
def perform(label_list, label_id_list, process_id, dataset_list, args, model, bg_text_features, fg_text_features, cam):
    cam_out_dir = r"D:\CDFSSDataset\Target domains\Lung Segmentation\Pascal"
...
if __name__ == "__main__":
    ...
    parser.add_argument('--img_root', type=str, default=r'D:\CDFSSDataset\Target domains\Lung Segmentation\CXR_png')
    ...
    label_list = ["lung X-ray"]
    label_id_list = [0]
    device = "cuda" if torch.cuda.is_available() else "cpu"
    train_list = [os.path.join(args.img_root, f) for f in os.listdir(args.img_root) if f.endswith('.png')]

    if not os.path.exists(args.cam_out_dir):
        os.makedirs(args.cam_out_dir)
    model, _ = clip.load(args.model, device=device)
    bg_text_features = zeroshot_classifier(BACKGROUND_CATEGORY_lung, ['a clean origami {}.'], model)#['a rendering of a weird {}.'], model)
    fg_text_features = zeroshot_classifier(new_class_names_lung , ['a clean origami {}.'], model)#['a rendering of a weird {}.'], model)
